...
База данных - это сильнопорезанная версия отладочной базы информационной системы. За счет очистки определенных таблиц, неиспользуемых в нашем стенде, размер базы с 90 Гб удалось уменьшить до 11 Гб. Размер же актуальной рабочей базы ИС - около 400 Гб
...
Стенд запускается примерно 7 минут

После создания и запуска ВМ нужно развернуть на них конфигурации с помощью ansible.
При первом запуске нам также будет нужно развернуть на сервер Database-1 дамп базы, поэтому, чтобы на Database-2 нормально запустилась репликация первый запуск плейбука делаем без Database-2
